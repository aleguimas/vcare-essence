...
14. **Revisão geral de SEO + AIO (2026-06-17).** `lib/schemas.ts`: `medicalClinicSchema` enriquecido com `areaServed`, `knowsAbout`, `founder` (refs aos `@id` dos profissionais), `paymentAccepted`, `isAcceptingNewPatients`, `slogan`, `sameAs` (quando houver Instagram) e `hasOfferCatalog` listando as 7 verticais (`SERVICE_OFFERS`). `/mep` e `/metodo-elo` ganharam `MedicalTherapy` JSON-LD (antes só breadcrumb); `/agendar` ganhou `FAQPage` + breadcrumb. `lib/seo.ts`: removida a OG estática inexistente (`/images/og/og-default.jpg`), passou a depender da `app/opengraph-image.tsx` (herdada por todas as rotas); keywords e `category` atualizadas. `app/sitemap.ts`: `/diario` removido (é `noindex`) e prioridade alta corrigida para `/mep` e `/metodo-elo`. `app/robots.ts`: bots de IA (GPTBot, ClaudeBot, OAI-SearchBot, PerplexityBot, Google-Extended etc.) explicitamente permitidos. Novo `public/llms.txt` (padrão llmstxt.org) descrevendo clínica, profissionais, métodos e contato só por WhatsApp. **Pendência conhecida:** "anos de prática" da Vanessa aparece como **15** em `/mep` e `/cuidados/hipnoterapia`, mas o CLAUDE.md diz **14**; confirmar com as sócias e unificar.
